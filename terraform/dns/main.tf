...
resource "aws_acm_certificate" "domain_certificate_us" {
  domain_name       = var.cert_subdomain != null ? "${var.cert_subdomain}.${var.zone_domain}" : var.zone_domain
  validation_method = "DNS"

  provider = aws.us-east-1 # Put cert in us-east-1 as that is what is usually required!
...
locals {
  verification_records = [for record in aws_acm_certificate.domain_certificate_us.domain_validation_options : record]
}
...
resource "aws_route53_record" "cert_verification" {
  ...
  depends_on = [
    aws_acm_certificate.domain_certificate_us,
  ]
